---
helpx_url: "https://helpx.adobe.com/substance-3d-sampler/technical-support/technical-issues/performance-issues/interface-lags-when-interacting-with-the-layer-stack-or-other-elements.html"
breadcrumb-title: ''
description: 이전 Wacom 드라이버 또는 기타 호환성 문제로 인한 Substance 3D Sampler의 인터페이스 지연 문제를 해결하는 방법을 알아봅니다.
helpx_creative_field: ""
helpx_description: Sampler > Technical Support > Technical Issues > Performance issues > Interface lags when interacting with the layer stack or other elements
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: 레이어 스택 또는 다른 요소와 상호 작용할 때 인터페이스가 지연됨
user-guide-description: ''
user-guide-title: ''
source-git-commit: 55277f7a92e97bf530dd2a2edf4e16c88bb57793
workflow-type: tm+mt
source-wordcount: '65'
ht-degree: 0%

---


# 레이어 스택 또는 다른 요소와 상호 작용할 때 인터페이스가 지연됨

이전 Wacom 드라이버가 설치된 경우 문제를 확인했습니다.

Wacom 드라이버를 업데이트하면 문제를 해결하고 샘플의 성능을 향상시킬 수 있습니다.
