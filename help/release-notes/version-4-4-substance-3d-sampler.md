...
breadcrumb-title: ''
description: 텍스트 텍스처 및 이미지 텍스처 기능을 포함한 생성 워크플로우에 대해 알아보려면 Substance 3D Sampler 버전 4.4의 릴리스 정보를 검토하십시오.
...
<b>Substance 3D Sampler 4.4</b>에서는 세 가지 새로운 생성 워크플로(텍스트-텍스처, 텍스트-패턴, 이미지-텍스처)를 beta로 도입했습니다.
...
텍스트 텍스처 기능을 사용하면 <b>텍스트 프롬프트</b>를 사용하여 재질을 만드는 새로운 방법을 탐색할 수 있습니다. 자세한 텍스트 설명에서 타일 텍스처를 생성하고 Image-to-Material 또는 Sampler 필터를 사용하여 원하는 방식으로 결과물을 만들어 사용할 수 있습니다.

## Image-to-texture

![이미지-텍스처](../assets/imagetoText_whatNewPanel.png "이미지-텍스처")

Image-to-Tile을 사용하면 정사각형이 아니고 타일링이 아니더라도 <b>자신의 참조 이미지</b>에서 타일링한 정사각형 텍스처를 만들 수 있습니다. 이렇게 하면 완벽한 프롬프트를 작성할 필요 없이 원하는 결과에 더 가까워집니다.\
이미지를 텍스처로 변환하면 이미 만든 콘텐츠에서 변형을 만들어 시간을 절약할 수도 있습니다.
...
* [생성형 인공지능] 이미지를 텍스처(Beta)로
* [생성형 인공지능] 텍스트로 패턴(베타)
* [생성형 인공지능] 텍스트를 텍스처(베타)로
* [스크립팅] 이제 에셋에 &#39;리소스&#39; 속성이 있습니다.
* [스크립팅] 이제 레이어에 &#39;출력\_사용량&#39; 속성이 있습니다.

<b>고정:</b>

* [Application] 손상된 프로젝트 파일을 열 때 충돌 발생
* [Application] 프로젝트에 손상된 에셋이 포함된 경우 충돌
* [응용 프로그램] Windows에서 모니터 연결 해제 시 충돌
...
* [Filters] 입력 레이어에 따라 확대 충돌 가능
* [생성형 인공지능] 서비스에서 예기치 않은 결과를 받았을 때 발생할 수 있는 충돌
* [스크립팅] 환경 변수에서 플러그인을 자동으로 로드할 때 충돌 발생
* [스크립팅] API를 사용하여 출력 사용을 할당할 때 가능한 충돌
