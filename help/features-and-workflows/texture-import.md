...
breadcrumb-title: ''
description: 텍스처를 Substance 3D Sampler으로 가져와 재질 제작 워크플로에서 기존 이미지 파일을 사용하는 방법을 알아봅니다.
...
채널 일치는 아래에 설명된 특정 이름 지정 규칙을 기반으로 합니다. 일치하는 항목이 없는 중복 또는 텍스처의 경우 인터페이스에서 이미지가 해당 항목으로 표시됩니다.

## OpenPBR

Sampler에서 다음 OpenPBR 식별자가 있는 파일을 해당 재질의 채널과 일치시킵니다.

>[!NOTE]
>
> Height 채널 식별자는 ASM에 사용되는 채널 채널과 동일합니다.
...
| subsurface_weight | subsurfaceWeight/반투명도 |
...
| **채널** | **Adobe Standard Material** |
| --- | --- |
| **앰비언트 오클루전** | <ul><li>양쪽융기</li><li>ao</li><li>오클루전</li><li>ambient_오클루전</li></ul> |
