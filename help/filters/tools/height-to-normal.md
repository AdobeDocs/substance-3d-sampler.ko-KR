...
**일반 필터로 Height**&#x200B;를 사용하면, 맨 위 이미지에 표시된 높이 맵에서 일반 데이터가 생성됩니다. 생성된 노멀 맵 덕분에 두 번째 이미지의 재질에서 빛이 더욱 사실적으로 반사됩니다.
...
* **세계 단위 사용**: 전환\
  ...
  * **세계 단위 사용이 활성화된 경우:**
    ...
    * **Height 깊이(cm)**: 0-10\
      Height 맵으로 나타내는 거리를 설정합니다. 높이 맵이 작은 거리를 나타낸다면 높이 맵 값의 차이가 크면 수직 각도에 작은 영향을 줄 수 있습니다. 높이 맵이 큰 거리를 나타내는 경우, 높이 맵 값의 작은 차이가 노멀 맵 상의 큰 각도를 나타낼 수 있다.
  * **세계 단위 사용이 비활성화된 경우:**
    * **강도**: 0-3\
      법선 각도의 경사를 조정합니다
* **아래쪽 표준 결합**: 0-1\
  이 필터의 결과에 기존 노멀 맵을 추가합니다.
